...
In `nuxt.config.ts`, map these public environment variables into runtime config keys used by the examples below:
...
In Remix, a route module such as `_index.tsx`, you can export a `loader`, an `action`, and a default component.
...
// Browser: create a client using the environment variables returned by the loader.
...
In React Router, a route module (`_index.tsx`) can export a `loader`, an `action`, and a default component. Create a server client inside the `loader` and `action`, and a browser client inside the component, passing the environment variables through the `loader`.
